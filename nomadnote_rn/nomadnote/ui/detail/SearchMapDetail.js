...
export default class SearchMapDetail extends Component {
    constructor(props) {
        super(props);
        SetI18nConfig();
    ...
    componentDidMount() {
        ...
        this.didFocusListener = this.props.navigation.addListener(
            'didFocus',
            async () => {
                let strWrite = await getItemFromAsync('map_write');
                if (strWrite === 'modi' || strWrite === 'remove') {
                    this.setState({ isLoading: true }, () => {
                        this.getPlaceWithPlaceId()
                    })
                } else {
                    this.getPlaceWithPlaceId()
                }
    ...
    getPlaceWithPlaceId() {
        let url = /*global.server*/ 'https://iloveseoyeon.com' + '/api/place/load_place'

        var bodyFormData = new FormData();
        bodyFormData.append('member_id', global.user.id);
        bodyFormData.append('place_id', this.state.id)
        ...
        axios.post(url, bodyFormData, {
            ...
            .then(function (response) {
                console.log('imageCheck', response.data)
                this.setState({
                    placeInfo: response.data.place,
                    backgroudImg: (response.data.place.image_uri === null || response.data.place.image_uri === '') ? '' : global.server + response.data.place.image_uri
                }, () => {
                    this.getPlaceTimeline();
                })
            }.bind(this))
            .catch(function (error) {
                if (error.response != null) {
    ...
    async getPlaceTimeline() {
        ...
        let url = /*global.server*/ 'https://iloveseoyeon.com' + '/api/timeline/place_timeline'
        ...
        console.log('place id: ', this.state.id)
    ...
    setScrap = (id) => {
        let url = /*global.server*/ 'https://iloveseoyeon.com' + '/api/timeline/set_scrap'
        ...
        axios.post(url, bodyFormData, {
            ...
            .then(function (response) {
                console.log('scrap in map: ', response.data)
                setItemToAsync('scrap_click', 'map')
    ...
    isCloseToBottom = ({ layoutMeasurement, contentOffset, contentSize }) => {
        if (contentOffset.y < SCREEN_WIDTH * 0.7) {
    ...
    formatDecimal(n) {
        let money = 0
        money = n.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        return money
    }
    ...
    render() {
        ...
        var convertTime = moment.utc(data.total_durations * 1000).format('HH:mm');
        console.log('converTime : ', convertTime)

        var placeSting = ''
        if (global.lang == 'ko') {
            placeSting = data.place
        } else if (global.lang == 'ja') {
            placeSting = data.ja
        } else if (global.lang == 'zh_rCN') {
            placeSting = data.zh_rCN
        } else if (global.lang == 'zh_rTW') {
            placeSting = data.zh_rTW
        } else if (global.lang == 'zh-Hant-MO') {
            placeSting = data.zh_rTW
        } else {
            placeSting = data.en
                    ...
                    <View style={styles.container}>
                        <LinearGradient colors={['#638d96', '#507781']} style={styles.linearGradient}>
                            <View style={{ justifyContent: 'center', alignItems: 'center', width: SCREEN_WIDTH, height: 50 }}>
                                ...
                                <Text style={styles.titleText}>{Translate('searchbymap')}</Text>
                            </View>
                            <View style={styles.searchContainer}>
                                <TextInput
                                    ...
                                    onSubmitEditing={() => this.getPlaceTimeline()}
                                />
                                <TouchableWithoutFeedback
                                    style={{ marginTop: 10 }}
                        ...
                        <View style={styles.imgContainer}>
                            {
                                (this.state.placeInfo === '' || this.state.placeInfo === null) ? <View style={{ backgroundColor: '#fff', width: '100%', height: '100%' }} /> : <ImageBackground
                                    style={styles.placeImg}
                                    source={this.state.backgroudImg === '' ? require('../../images/time_bg.png') : { uri: this.state.backgroudImg }}>
                                    <View style={styles.imgBackgroundContainer}>
                                        ...
                                        <View style={{
                                            ...
                                            <View style={styles.timeContainer}>
                                                <Text style={styles.timeTitleText}>{Translate('duration_time')}</Text>
                                                <Text style={styles.timeText}>{`${convertTime}(h:m)`}</Text>
                                            </View>
                                            <View style={{
                                                width: 2,
                                                height: '92%',
                                            ...
                                            <View style={styles.amountContainer}>
                                                <Text style={styles.amountTitleText}>{Translate('consumption_amount')}</Text>
                                                <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'center' }}>
                                                    <Text style={{
                                                        fontSize: normalize(18),
                                                        fontWeight: '500',
                                                        color: '#fff',
                                                    }}>{this.formatDecimal(Number(data.total_costs))}</Text>
                                                    <Text style={{
                                                        fontSize: normalize(18),
                                                        fontWeight: '500',
                                                        color: '#fff',
                            ...
                            {/* <ScrollView
                                onScroll={({ nativeEvent }) => {
                                    if (this.onScroll(nativeEvent)) {
                                        ...
                                        if (this.state.page <= this.state.placeEndPage) {
                                    ...
                                    renderItem={({ item, index }) => (
                                        <TouchableWithoutFeedback
                                            ...
                                            <View style={styles.cardContainer}>
                                                    ...
                                                    <View style={{ marginLeft: SCREEN_WIDTH * 0.0417, marginTop: SCREEN_WIDTH * 0.667 * 0.089, marginRight: (SCREEN_WIDTH * 0.3194) + (SCREEN_WIDTH * 0.0333) + 8 }}>
                                                        <View style={{ width: '100%', flexDirection: 'row', alignItems: 'flex-start', marginBottom: SCREEN_WIDTH * 0.01389 }}>
                                                            <Image
                                                                ...
                                                                source={item.member.profile == null ? item.member.gender === 'F' ? require('../../images/ic_female.png') : require('../../images/ic_male.png') : { uri: /*global.server*/ 'https://iloveseoyeon.com' + item.member.profile }} />
                                                    ...
                                                    <View style={{ borderRadius: 6, backgroundColor: '#d8d8d8', width: SCREEN_WIDTH * 0.3194, height: SCREEN_WIDTH * 0.667 * 0.3348, marginRight: SCREEN_WIDTH * 0.0333, position: 'absolute', right: 0, top: 0, marginTop: SCREEN_WIDTH * 0.0333, alignSelf: 'flex-end' }}>
                                                        {
                                                            item.images.length > 0 && item.images[0].image_uri === null ?
                                                                ...
                                                                : item.youtube === null ?
                                                                    <Image
                                                                        style={{ width: '100%', height: '100%', borderRadius: 6, }}
                                                                        source={item.images.length > 0 ? { uri:  /*global.server*/ 'https://iloveseoyeon.com' + item.images[0].image_uri } : require('../../images/time_bg.png')}
                                                                    />
                                                ...
                                                <Text
                                                    style={{ fontSize: normalize(12), color: '#4a4a4a', marginBottom: 2, marginHorizontal: SCREEN_WIDTH * 0.04167, flex: 1, marginTop: SCREEN_WIDTH * 0.667 * 0.0565 }}
                                                ...
                                                <View style={styles.elemTravel}>
                                                    <View style={styles.elemSetting}>
                                                        ...
                                                        <View style={{
                                                            ...
                                                            <Text adjustsFontSizeToFit style={{ alignSelf: 'center', color: item.style_id === 2 ? '#ffffff' : '#878787', textAlign: 'center', fontSize: this.state.lang === 'ja' ? normalize(8) : this.state.lang === 'en' ? normalize(9) : normalize(10), marginHorizontal: 1, marginVertical: 4 }}>{Translate('activity_write_hotplace')}</Text>
                                                        </View>
                                                        <View style={{
                                                            marginLeft: this.state.lang === 'en' ? 1 : 5, borderRadius: 2, flex: 1 / 6,
